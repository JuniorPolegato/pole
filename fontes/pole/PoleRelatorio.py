...
CABECALHO = ("<br/>"
              "<b>Pole - Python utilities by Junior Polegato</b><br/>"
              "This module combines functions for development on a day-to-day<br/>"
              "Facilitate the use of GTK + for GUI and ReportLab for PDF<br/>"
...
def gerar_pdf(paisagem, data_inicial, data_final, titulo_relatorio,
    ...
    try:
        ...
        if not dados:
            ...
            registros = [linha[0] for linha in dados]
        else:
            registros = dados
    # Imagem do logo
    ...
    pdf.celula(10 * cm , I.drawHeight, None, [PolePDF.Paragraph(cabecalho_logo, PolePDF.pequena_centro)], borda = False)
    ...
    altura_disponivel -= pdf.celula(largura_folha, 1, None,
                                    [PolePDF.Paragraph(titulo_relatorio, grande_esquerda)], borda = False)[3]
    altura_disponivel -= pdf.celula(largura_folha, 1, None,
                                    [PolePDF.Paragraph(data_inicial + " à " + data_final, normal_esquerda)],
                                    borda = False)[3]
    # Pula 0,5 cm
    altura_disponivel -= pdf.celula(largura_folha, 0.5 * cm, '', '', borda = False)[3]
    ...
    dados_tabela.append([PolePDF.Paragraph("Totais", PolePDF.normal_esquerda)] + [''] * (len(tipos) - 1))
    ...
    for n, t in enumerate(totalizacao):
        if t.lower()[:4] in ('perc', 'porc'):
            ...
            totalizado[n] = formatar((a / b) * 100, t)
    # Inclusão do totalizado no final dos dados
    dados_tabela.append([Paragraph(t, e) for t, e in zip(totalizado, estilos)])
    ...
    for pagina, (tabela, alt_tab) in enumerate(tabelas):
        ...
        pdf.celula(largura_folha, 1, None,
                   [PolePDF.Paragraph(str_pagina % cf(pagina, int)[1], PolePDF.pequena_direita)],
                   borda = False, posicao = (1 * cm, 0.55 * cm))
        pdf.celula(largura_folha, 1, None,
                   [PolePDF.Paragraph(str_agora, PolePDF.pequena_direita)],
                   borda = False, posicao = (1 * cm, 0.73 * cm))
        # Cabeçalho das colunas
        for largura, cabecalho, estilo in zip(larguras, cabecalhos, estilos):
            pdf.celula(largura, altura_cabecalho, None, [PolePDF.Paragraph(cabecalho, estilo)])
        # Tabela
