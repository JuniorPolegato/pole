...
'''
    ...
    def download_nfe(self, chave):  # usar uf = 'AN' (ambiente nacional)
        ...
        requisicao.distDFeInt['xmlns'] = 'http://www.portalfiscal.inf.br/nfe'
        requisicao.distDFeInt['versao'] = '1.00'
        requisicao.distDFeInt.tpAmb = self.__ambiente
        requisicao.distDFeInt.cUFAutor = 35  # self.__uf
        requisicao.distDFeInt.CNPJ = self.__cnpj
        requisicao.distDFeInt.distNSU.ultNSU = "%015i" % 1
        # requisicao.distDFeInt.consChNFe.chNFe = chave
        xml_download = self.servico('NfeDistribuicaoDFe', requisicao,
                                    versao_wsdl='1.00')
        nfe_zip = xml_download.retDistDFeInt.loteDistDFeInt.docZip
        nfe = zlib.decompress(base64.decode(nfe_zip))
        return PoleXML.importar(nfe)
